...
SetupFolders() {
    ...
    ; Get Downloads folder - use environment variable for reliability
    ...
    if (scriptPath = newScriptPath) {
        ; Already in correct location, just ensure subfolders exist
    ...
    ; First run - create folders and move
    try {
        ...
        ; Copy script to Downloads\ReHelicity
        ...
        ; If it fails, just use current directory
...
; LOGO SETUP
; ============================================
global LogoFile := DataFolder . "\helicity_logo.png"
...
CreateHelicityLogo() {
    ...
    try {
        ; Download logo from imgur or create it
        ...
        ; Try to download
        try {
            Download(logoUrl, LogoFile)
        } catch {
            ; If download fails, that's okay - we'll just not use a logo
        }
    }
}
...
global Settings := {
    ...
    SpawnDelay: 4,
    KeyPressDelay: 150,
    DisconnectWaitTime: 12,
    BloxstrapKillDelay: 3,
...
ShowGoodServerPopup(ocrResult) {
    global Running, Attempts, LogoFile
    ...
    popupGui.Add("GroupBox", "x20 y70 w460 h160 c0x555555", "Server Details")
    popupGui.Add("Text", "x40 y95 w420 h130 c0xcccccc", details)
    
    popupGui.SetFont("s11", "Segoe UI")
    popupGui.Add("Text", "x20 y220 w460 h30 c0xFFFFFF Center", "Would you like to continue rerolling?")
    
    popupGui.SetFont("s9", "Segoe UI")
    timerText := popupGui.Add("Text", "x20 y255 w460 h25 c0x888888 Center", "If not closed will continue to reroll in 15 minutes (18:51)")
    
    popupGui.SetFont("s11 bold", "Segoe UI")
    btnContinue := popupGui.Add("Button", "x40 y290 w200 h45 Background0x2a7a2a c0xffffff", "Continue rerolling")
    btnClose := popupGui.Add("Button", "x260 y290 w200 h45 Background0x7a2a2a c0xffffff", "Close and wait")
    ...
    popupGui.Show("w500 h360 x400 y300")
    
    ; Store hwnd immediately after showing
...
ContinueRerolling(popupGui) {
    ...
    ; Close current game before rerolling
    QuickLeaveGame()
    Sleep(2000)
    CloseRoblox()
    
    SetTimer(RunRerollerLoop, 1000)
}
...
ReadThermosOCR() {
    ...
    try {
        ...
        LogToGUI("OCR Error: " . err.Message . " (Code: " . err.Extra . ")")
...
ParseThermosText(text) {
    ...
    LogToGUI("Parsing (first 150 chars): " . SubStr(text, 1, 150) . "...")
    
    patterns := ["RISK[:\s]*(\w+)", "RISK\s+(\w+)", "Risk:\s*(\w+)"]
    
    for pattern in patterns {
        if (RegExMatch(textUpper, pattern, &match)) {
            riskValue := Trim(match[1])
            
            if (InStr(riskValue, "HIGH"))
                result.risk := "HIGH"
            else if (InStr(riskValue, "MODERATE"))
                result.risk := "MODERATE"
            else if (InStr(riskValue, "SLIGHT"))
                result.risk := "SLIGHT"
            else if (InStr(riskValue, "ENHANCED"))
                result.risk := "ENHANCED"
            else if (InStr(riskValue, "MARGINAL"))
                result.risk := "MARGINAL"
            else if (InStr(riskValue, "TSTM"))
                result.risk := "TSTM"
            
            if (result.risk != "") {
                LogToGUI("Found risk: " . result.risk)
                break
            }
        }
    }
    
    if (RegExMatch(textUpper, "LAPSE\s+RATES?\s+(\d+\.?\d*)", &match)) {
        result.lapseRate := Float(match[1])
        LogToGUI("Found lapse rate: " . result.lapseRate . "°C")
    }
    
    if (RegExMatch(textUpper, "DEW\s+POINT\s+(\d+)", &match)) {
    ...
    if (RegExMatch(textUpper, "WIND\s+SHEAR\s+(\d+)", &match)) {
    ...
    if (!result.success) {
        LogToGUI("Failed to parse any values!")
    }
...
; ============================================
; SPAWN DETECTION FUNCTION
; ============================================

WaitForSpawn(timeoutSeconds := 30) {
    global TesseractPath, DataFolder
    
    LogToGUI("Waiting for spawn...")
    startTime := A_TickCount
    
    Loop {
        if ((A_TickCount - startTime) > (timeoutSeconds * 1000)) {
            LogToGUI("Timeout waiting for spawn")
            return false
        }
        ...
        try {
            ...
            ; Capture bottom center area where hotbar appears
            captureX := wx + (ww * 0.35)
            captureY := wy + (wh * 0.85)
            captureW := Round(ww * 0.3)
            captureH := Round(wh * 0.1)
            
            imageFile := DataFolder . "\spawn_capture.png"
            outputFileBase := DataFolder . "\spawn_ocr"
            outputTxtFile := outputFileBase . ".txt"
            ...
            if (FileExist(outputTxtFile)) {
                ...
                ; Check for "Flashlight" or "Probe" text
                if (InStr(textUpper, "FLASHLIGHT") || InStr(textUpper, "PROBE")) {
                    LogToGUI("Spawn detected!")
                    return true
                }
            }
            
        } catch as err {
            LogToGUI("Spawn detection error: " . err.Message)
        }
...
SendWebhook(ocrResult) {
    ...
    try {
        ...
        ; Create rich embed with purple color (8663711 = #8438FF purple)
        currentTime := FormatTime(, "yyyy-MM-dd'T'HH:mm:ss'Z'")
        
        embedJson := '{'
        
        ; Add content for pings if any
        if (pingContent != "")
        ...
        embedJson .= '"image": {"url": "attachment://thermos.png"}'
        embedJson .= '}]}'
        
        ; Read the thermos image and convert to base64
        imageFile := DataFolder . "\thermos_capture.png"
        
        if (FileExist(imageFile)) {
            ; Use multipart/form-data to send image with embed
            boundary := "----WebKitFormBoundary" . A_TickCount
            
            ; Read image file as binary
            imageData := FileRead(imageFile, "RAW")
            ...
            body .= embedJson . "`r`n"
            body .= "--" . boundary . "`r`n"
            body .= "Content-Disposition: form-data; name=`"files[0]`"; filename=`"thermos.png`"`r`n"
            body .= "Content-Type: image/png`r`n`r`n"
            
            ; Convert to proper format for sending
            bodyBuffer := Buffer(StrLen(body) + imageData.Size + 100)
            StrPut(body, bodyBuffer, "UTF-8")
            
            ; This is simplified - for full implementation, you'd need to properly encode the binary data
            ; For now, we'll send without the image attachment
...
WaitForMainMenu(timeoutSeconds := 60) {
    global TesseractPath, DataFolder, Settings
...
; ============================================
; GUI FUNCTIONS - TABBED INTERFACE
; ============================================

CreateGUI() {
    global MainGui, LogBox, StatusText, AttemptText, TesseractPath, LogoFile
    
    MainGui := Gui("+Resize", "Helicity Reroller v10.1 - Menu Detection")
    MainGui.BackColor := "0x0a0a0a"
    MainGui.SetFont("s10", "Segoe UI")
    
    ; Try to set custom icon if logo exists
    try {
        if (FileExist(LogoFile)) {
            ; Convert PNG to ICO format or use direct
            ; Note: AHK v2 requires .ico format for icons
            ; For now, we'll try to use it directly
        }
    }
    
    MainGui.SetFont("s14 bold", "Segoe UI")
    MainGui.Add("Text", "x20 y20 w760 cWhite Center", "ReHelecity")
    MainGui.SetFont("s10", "Segoe UI")
    ...
    btnCalibrate := MainGui.Add("Button", "x60 y270 w330 h30 Background0x2a4a7a c0xffffff", "⚙ Calibrate OCR (F5)")
    btnCalibrate.OnEvent("Click", (*) => CalibrateOCRRegion())
    
    btnTest := MainGui.Add("Button", "x410 y270 w330 h30 Background0x2a4a7a c0xffffff", "🔍 Test OCR (F4)")
    ...
    MainGui.Add("Text", "x410 y" . yPos . " w50 c0x888888", "°C")
    ...
    MainGui.Add("Text", "x410 y" . yPos . " w50 c0x888888", "kts")
    ...
    MainGui.Add("Text", "x410 y" . yPos . " w80 c0x888888", "J/Kg")
    ...
    MainGui.Add("Text", "x410 y" . yPos . " w80 c0x888888", "°F")
    ...
    MainGui.Add("GroupBox", "x40 y290 w720 h180 c0x555555", "Discord Webhook")
    MainGui.Add("Text", "x60 y320 w120 c0xcccccc", "Webhook URL:")
    editWebhook := MainGui.Add("Edit", "x180 y317 w550 h25 Background0x1a1a1a c0xffffff", Settings.WebhookURL)
    editWebhook.OnEvent("Change", (*) => (Settings.WebhookURL := editWebhook.Value, SaveSettings()))
    
    MainGui.Add("Text", "x60 y355 w120 c0xcccccc", "User ID (ping):")
    editUserID := MainGui.Add("Edit", "x180 y352 w550 h25 Background0x1a1a1a c0xffffff", Settings.WebhookUserID)
    editUserID.OnEvent("Change", (*) => (Settings.WebhookUserID := editUserID.Value, SaveSettings()))
    
    MainGui.Add("Text", "x60 y390 w120 c0xcccccc", "Role ID (ping):")
    editRoleID := MainGui.Add("Edit", "x180 y387 w550 h25 Background0x1a1a1a c0xffffff", Settings.WebhookRoleID)
    editRoleID.OnEvent("Change", (*) => (Settings.WebhookRoleID := editRoleID.Value, SaveSettings()))
    
    chkWebhook := MainGui.Add("Checkbox", "x60 y425 w200 c0xcccccc", "Enable Webhook Notifications")
    chkWebhook.Value := Settings.WebhookEnabled
    chkWebhook.OnEvent("Click", (*) => (Settings.WebhookEnabled := chkWebhook.Value, SaveSettings()))
    
    MainGui.Add("GroupBox", "x40 y480 w720 h140 c0x555555", "Timing Settings")
    
    MainGui.Add("Text", "x60 y510 w180 c0xcccccc", "Key Press Delay (ms):")
    editKeyDelay := MainGui.Add("Edit", "x240 y507 w80 h25 Background0x1a1a1a c0xffffff", Settings.KeyPressDelay)
    ...
    editWaitJoin := MainGui.Add("Edit", "x240 y542 w80 h25 Background0x1a1a1a c0xffffff", Settings.InitialWaitAfterJoin)
    editWaitJoin.OnEvent("Change", (*) => ValidateIntegerInput(editWaitJoin, "InitialWaitAfterJoin"))
    
    MainGui.Add("Text", "x60 y580 w180 c0xcccccc", "Spawn Delay (sec):")
    editSpawnDelay := MainGui.Add("Edit", "x240 y577 w80 h25 Background0x1a1a1a c0xffffff", Settings.SpawnDelay)
    editSpawnDelay.OnEvent("Change", (*) => ValidateIntegerInput(editSpawnDelay, "SpawnDelay"))
    
    MainGui.Add("GroupBox", "x40 y630 w720 h100 c0x555555", "OCR Region")
    MainGui.Add("Text", "x60 y660 w660 c0xcccccc", "Calibrated: " . (Settings.OCRRegion.w > 0 ? Settings.OCRRegion.w . "x" . Settings.OCRRegion.h . " pixels" : "Not calibrated"))
    MainGui.Add("Text", "x60 y685 w660 c0x888888", "Use F5 to calibrate the OCR capture region")
    ...
    MainGui.Add("Text", "x20 y760 w600 c0x666666", "F2=Start | F3=Stop | F4=Test OCR | F5=Calibrate OCR")
    MainGui.SetFont("s10", "Segoe UI")
    
    MainGui.Show("w800 h800")
    
    LogToGUI("=== Helicity Reroller Initialized ===")
    LogToGUI("Tesseract OCR Edition with Menu Detection")
    ...
    if (Settings.OCRRegion.w > 0) {
        LogToGUI("OCR region calibrated: " . Settings.OCRRegion.w . "x" . Settings.OCRRegion.h)
    } else {
        LogToGUI("WARNING: OCR region not calibrated!")
        LogToGUI("Click 'Calibrate OCR' button (F5)")
    }
}
...
CalibrateOCRRegion() {
    LogToGUI("=== OCR REGION CALIBRATION ===")
    LogToGUI("Step 1: Open Roblox and display thermos")
    LogToGUI("Step 2: In 10 sec, move mouse to TOP-LEFT of thermos")
    LogToGUI("")
    
    ShowCountdown(10)
    
    MouseGetPos(&x1, &y1)
    LogToGUI("Top-left: " . x1 . ", " . y1)
    SoundBeep(800, 200)
    
    LogToGUI("")
    LogToGUI("Step 3: In 10 sec, move mouse to BOTTOM-RIGHT")
    LogToGUI("")
    
    ShowCountdown(10)
    
    MouseGetPos(&x2, &y2)
    LogToGUI("Bottom-right: " . x2 . ", " . y2)
    SoundBeep(1000, 200)
    
    if (x1 >= x2 || y1 >= y2) {
        LogToGUI("ERROR: Invalid coordinates!")
        SoundBeep(400, 500)
        return
    }
    
    Settings.OCRRegion.x := x1
    Settings.OCRRegion.y := y1
    Settings.OCRRegion.w := x2 - x1
    Settings.OCRRegion.h := y2 - y1
    
    SaveSettings()
    
    LogToGUI("")
    LogToGUI("OCR REGION CALIBRATED!")
    LogToGUI("Size: " . Settings.OCRRegion.w . "x" . Settings.OCRRegion.h)
    LogToGUI("Now click 'Test OCR' (F4)!")
...
StartReroller() {
    ...
    if (Settings.OCRRegion.w <= 0) {
        LogToGUI("ERROR: Calibrate OCR first (F5)!")
        SoundBeep(400, 500)
        return
    }
...
RunRerollerLoop() {
    ...
    try {
        ...
        if (!ocrResult.success) {
            LogToGUI("OCR failed: " . ocrResult.error)
            QuickLeaveGame()
            Sleep(2000)
            CloseRoblox()
            if (Running)
        ...
        LogToGUI("Results: Risk=" . ocrResult.risk . " | Lapse=" . ocrResult.lapseRate . " | Shear=" . ocrResult.windShear . " | CAPE=" . ocrResult.cape . " | Dew=" . ocrResult.dewPoint)
...
ExecuteFullSequence() {
    ...
    ; Immediately start sequence after menu detection
...
CheckConditions(ocrResult) {
    ...
    ; Check ALL enabled filters - must pass all to return true
    if (Settings.LapseRate.enabled) {
...
TestThermosRead() {
    ...
    if (Settings.OCRRegion.w <= 0) {
        LogToGUI("ERROR: Calibrate OCR first!")
        return
    }
    ...
    if (!ocrResult.success) {
        LogToGUI("OCR Test FAILED: " . ocrResult.error)
        return
    }
